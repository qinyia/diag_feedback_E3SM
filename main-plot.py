...
datadir = '/global/homes/q/qinyi/scripts/CloudFeedback_scripts/'
...
# generally, if you want all lines in the same plot, just set ncase = [7]
#ncase = [3,6,len(cases)]
ncase = [7]
# ---------- optional settings ----------------
...
datadir_Ringer = datadir_CMIPs+'Ringer2014/'
...
if plot_CRE_globalmean:
    ...
    models_cmip6 = ['E3SM-1-0','BCC-CSM2-MR','CNRM-CM6-1','IPSL-CM6A-LR',\
    'MRI-ESM2-0','GISS-E2-1-G','CESM2','GFDL-CM4']
    models_cmip5 = ['bcc-csm1-1','CNRM-CM5','IPSL-CM5A-LR','IPSL-CM5B-LR',\
    'FGOALS-g2','MIROC5','MPI-ESM-LR','MPI-ESM-MR','MRI-CGCM3','CCSM4']
    models_all = models_cmip6 + models_cmip5
    
    for model in models_all:
        if model in models_cmip5:
            filename = datadir_Ringer+'global_mean_features_amip_'+model+'_r1i1p1.csv'
        else:
            if model == 'CNRM-CM6-1':
                filename = datadir_Ringer+'global_mean_features_amip_'+model+'_r1i1p1f2.csv'
            else:
                filename = datadir_Ringer+'global_mean_features_amip_'+model+'_r1i1p1f1.csv'
    ...
    for icase,case in enumerate(cases):
        if case == 'v1_coupled':
            ...
            df1 = pd.read_csv(datadir_v2+'global_mean_features_amip_E3SM-1-0_'+case+'.csv',index_col=0)
    ...
    for idx,index in enumerate(df_plot.index):
        ...
        for icol,column in enumerate(df_others_plot.columns):
            ax.scatter(x[idx]-0.2,df_others_plot.loc[index,column].tolist(),edgecolor='none',facecolor='grey',alpha=a1,s=s2)
        # ensemble mean
        ax.scatter(x[idx]-0.2,df_others_plot.loc[index,:].mean().tolist(),edgecolor='black',facecolor='black',s=s2)
...
if plot_RadKernel_globalmean:
    ...
    for icase,case in enumerate(cases):
        if case == 'v1_coupled':
            # read v1-coupled 
            df_coupled = pd.read_csv(datadir_v1+'FDBK_CMIP6_piControl_E3SM-1-0.csv',index_col=0)
            df_all['v1_coupled'] = df_coupled.iloc[:26,0]
        elif case == 'v1_amip':
            # read v1-amip
            df_amip = pd.read_csv(datadir_v1+'FDBK_CMIP6_amip_E3SM-1-0.csv',index_col=0)
            df_all['v1_amip'] = df_amip.iloc[:26,:]
        else:    
            df1 = pd.read_csv(datadir_v2+'FDBK_amip_E3SM-1-0_'+case+'.csv',index_col=0)
...
if plot_RadKernel_zonalmean:
    ...
    variables = ['SWCRE_ano_grd_adj_gfdbk','LWCRE_ano_grd_adj_gfdbk','netCRE_ano_grd_adj_gfdbk']
    ...
    for ii in ncase:
        ...
        for ivar,svar in enumerate(variables):
            ...
            for icase,case in enumerate(cases[:ii]):
                if case == 'v1_coupled':
                    f1 = cdms.open(datadir_v1+'lat-lon-gfdbk-CMIP6-piControl-E3SM-1-0.nc')
                elif case == 'v1_amip':
                    f1 = cdms.open(datadir_v1+'lat-lon-gfdbk-CMIP6-amip-E3SM-1-0.nc')
                else:
                    f1 = cdms.open(datadir_v2+'lat-lon-gfdbk-amip-E3SM-1-0-'+case+'.nc')
...
if plot_CldRadKernel_globalmean:
    ...
    for icase,case in enumerate(cases):
        if case == 'v1_coupled':
            ...
            df1 = pd.read_csv(datadir_v2+'decomp_global_mean_lw_amip_E3SM-1-0_'+case+'.csv',index_col=0)
            df2 = df1.loc[:,'E3SM-1-0']
            df_LW_all[case] = df2
    
            df1 = pd.read_csv(datadir_v2+'decomp_global_mean_sw_amip_E3SM-1-0_'+case+'.csv',index_col=0)
            df2 = df1.loc[:,'E3SM-1-0']
            df_SW_all[case] = df2
    ...
    for ii in range(3): ## loop for each panel, ii reprents All, Non-Low and Low Cloud 
        for icol,column in enumerate(df_LW_all.columns):
            ...
            if column == 'v1_coupled':
                L1 = axes[ii].scatter(x-w+w2,y1.values.tolist(),marker='x',s=s1,color=colors[icol],linewidths=wf,alpha=a1,label=column)
            elif column == 'v1_amip':
        ...
        for icol,column in enumerate(df_net_all.columns):
            ...
            if column == 'v1_coupled':
                L2 = axes[ii].scatter(x+w2,y1.values.tolist(),marker='x',s=s1,color=colors[icol],linewidths=wf,alpha=a1,label=column)
            elif column == 'v1_amip':
        ...
        for icol,column in enumerate(df_SW_all.columns):
            ...
            if column == 'v1_coupled':
                L3= axes[ii].scatter(x+w+w2,y1.values.tolist(),marker='x',s=s1,color=colors[icol],linewidths=wf,alpha=a1,label=column)
            elif column == 'v1_amip':
                axes[ii].scatter(x+w+w2,y1.values.tolist(),marker='x',s=s1,color=colors[icol],linewidths=wf,alpha=a1,label=column)
            else:
                axes[ii].scatter(x+w+w2,y1.values.tolist(),marker='o',s=s2,color=colors[icol],linewidths=wf,alpha=a1,label=column)
                ...
                for decomp in decomps:
                    ...
                    for icase,case in enumerate(cases[:ii]):
                        if case == 'v1_coupled':
                            ...
                            f1 = cdms.open(datadir_v2+'global_cloud_feedback_amip_E3SM-1-0_'+case+'.nc')
