...
outdir_final = curdir+'/data_diffyrE/'
# set output figure directory
figdir = curdir+'/figure_diffyrE/'
...
for icase,case in enumerate(case_stamp):
    ...
    for yearE2 in range(3,7):
        ...
        if PreProcess:
            ...
            os.system('sh get_data.sh '+ run_id1 + ' '+run_id2+' '+rgr_map+' '+str(yearS1)+' '+str(yearE2)+' '+str(yearS2)+' '+str(yearE2)+' '+datadir_in1+' '+datadir_in2+' '+outdir_out+' '+comp)
        ...
        if RunDiag:
            ...
            dics_cal = AP.get_cal_dics(direc_data, case_stamp[icase]+'_'+str(yearS2)+'yr-'+str(yearE2)+'yr', yearS2, yearE2, run_id1, run_id2, outdir_final,
