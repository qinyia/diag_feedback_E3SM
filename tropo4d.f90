subroutine tropo4d(temp, nlon, nlat, nlev, ntime, pres, plimu, pliml, plimlex, dofill, tp, tperr)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Oct 13, 2021: revise it to fit 4D data [time,lev,lat,lon] - Yi Qin
! input:    temp(nlon,nlat,nlev,ntime)    4D-temperature field
!           ntime                         # of time steps
! output:   tp(nlon, nlat, ntime)         tropopause pressure in Pa, same horizontal dimension as temp
!
...
real,intent(in),dimension(nlon,nlat,nlev,ntime) :: temp
real,intent(in),dimension(nlev)                 :: pres
real, intent(in)                                :: plimu, pliml, plimlex
logical, intent(in)                             :: dofill
real,intent(out),dimension(nlon,nlat,ntime)     :: tp
...
do time=1,ntime
  if (invert.eq.1) then
    do i=1,nlev
      t(i)=temp(lon,lat,nlev+1-i,time)
    enddo
  else
    do i=1,nlev
      t(i)=temp(lon,lat,i,time)
    enddo
  endif
...
if (dofill) then
  do time=1,ntime
    call fill(tp(:,:,time), nlon, nlat, ifil)
