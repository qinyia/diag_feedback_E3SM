...
for yrE in [2010]:
 ...
 #           run_id2 = CLP.get_lutable(case,'P4K_FR')
            run_id1 = CLP.get_lutable(case,'PI')
            run_id2 = CLP.get_lutable(case,'PD')
        ...
        if PreProcess:
            ...
            os.system('sh get_data_select_ERFaci.sh '+ run_id1 + ' '+run_id2+' '+rgr_map+' '+str(yearS_CTL)+' '+str(yearE_CTL)+' '+str(yearS_P4K)+' '+str(yearE_P4K)+' '+datadir_in1+' '+datadir_in2+' '+outdir_out+' '+comp)
